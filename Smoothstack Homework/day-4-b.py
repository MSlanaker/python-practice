...
total = list(map(lambda x: x if x[1] >= min_amount else (x[0], x[1] + 10),
...
invoice_totals = list(
    map(lambda x: [x[0]] + list(map(lambda y: y[1]*y[2], x[1:])), orders))
invoice_totals = list(
    map(lambda x: [x[0]] + [reduce(lambda a, b: a + b, x[1:])], invoice_totals))
invoice_totals = list(
    map(lambda x: x if x[1] >= min_amount else (x[0], x[1] + 10), invoice_totals))
